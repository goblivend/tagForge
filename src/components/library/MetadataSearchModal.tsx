...
function MetadataSearchBody({
    ...
    const panel = (
        <div className={inline ? `flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--panel-shadow)] ${className || ''}` : 'bg-card w-full max-w-2xl rounded-xl shadow-lg border border-border flex flex-col max-h-[90vh] overflow-hidden'}>
            <div className={`flex items-center justify-between p-4 border-b border-border ${inline ? 'bg-muted/20' : 'bg-muted/30'}`}>
...
export function MetadataSearchModal(props: MetadataSearchModalProps) {
    return <MetadataSearchBody {...props} />;
}
